...
class _PostDetailsState extends State<PostDetails> {
  ...
  Future<void> fetchPosts() async {
    ...
    for (String postId in postIDList) {
      ...
      if (postSnap.exists) {
        if (postSnap.data()?['Uploaded UID'] == widget.UID) {
          postUIDList.add(postSnap.data()?['Uploaded UID']);
          matchingPostIds.add(postSnap.data()?['postid']);
          matchingpostfollowersonly.add(postSnap.data()?['Followers Only']);
    ...
    setState(() {
      ...
      followersonly = matchingpostfollowersonly;
      PostUIDS=postUIDList;
    });
    if (kDebugMode) {
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                       ...
                       PostUIDS[index]==_auth.currentUser!.uid? InkWell(
                          onTap: ()async{
                            // print(PIDS[index]);
                            if(followersonly[index]){
                              await _firestore.collection('Global Post').doc(PIDS[index]).update(
                                  {
                            ...
                            setState(() {
                              followersonly[index]=!followersonly[index];
                            });
                          },
                          child: Container(
                            child: followersonly[index]? const Icon(Icons.star,color: Colors.green,):const Icon(Icons.people,color: CupertinoColors.white,),
                          ),
                        ):Container(),
                      ],
                    ),
