...
''';
  ...
  Future<void> fetchdevicedetails() async {
    ...
    if (defaultTargetPlatform == TargetPlatform.android) {
      ...
      setState(() {
        ...
        _devicemodelname=androidInfo.model;
        _devicetype='Android';
      });
      if (kDebugMode) {
  ...
  Future<void> writesessiondetails() async {
    await generatePostID();
    await fetchdevicedetails();
    await _getAreaName();
    await _firestore.collection('Session IDs').doc(_auth.currentUser!.uid).set(
        {
          'Session ID': FieldValue.arrayUnion([randomFiveDigitNumber.toString()])
        }, SetOptions(merge: true)
    );

    await _firestore.collection('Session Details').doc(randomFiveDigitNumber.toString()).set(
        {
          'Session ID': randomFiveDigitNumber.toString(),
          'Device ID': _devicename,
          'Area': _areaName,
          'Device Type':_devicetype,
          'Country Name': _countryName,
          'Active': false,
          'Model Name':_devicemodelname,
  ...
  final TextEditingController _EmailController=  TextEditingController();
   ...
   Future<void> loginuser()async{
     try{
       ...
       await checkwhethertowrite(_auth.currentUser!.uid);
       if(_iswrite){
         await writesessiondetails();
       }
                      ...
                      child: Center(
                        child:isloading? const SizedBox(
                          ...
                          child: CircularProgressIndicator(
                            ...
                            : Text('Login',style: GoogleFonts.poppins(color: Colors.white,fontWeight: FontWeight.w500),),
                      ),
                    ),
                  ),
                  const SizedBox(
